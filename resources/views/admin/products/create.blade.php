...
<div class="mb-6 md:mb-8">
    ...
    <h1 class="text-2xl md:text-3xl font-bold text-gray-800 mt-3 md:mt-4">Tambah Produk Baru</h1>
</div>
    ...
    <form action="/admin/products" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4 md:mb-6">
            ...
            <input type="text" name="name" required value="{{ old('name') }}"
                   class="w-full px-3 md:px-4 py-2 md:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-800 focus:border-red-800 text-sm md:text-base @error('name') @enderror">
        ...
        <div class="mb-4 md:mb-6">
            ...
            <textarea name="description" rows="4"
                      class="w-full px-3 md:px-4 py-2 md:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-800 focus:border-red-800 text-sm md:text-base">{{ old('description') }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 mb-4 md:mb-6">
            <div>
                ...
                @error('price')<p class="text-red-500 text-xs md:text-sm mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-gray-700 font-semibold mb-2 text-sm md:text-base">Stok <span class="text-red-500">*</span></label>
                <input type="number" name="stock" required value="{{ old('stock', 0) }}" min="0"
                       class="w-full px-3 md:px-4 py-2 md:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-800 focus:border-red-800 text-sm md:text-base">
            </div>
        </div>
        ...
        <!-- Image -->
        <div class="mb-4 md:mb-6">
            <label class="block text-gray-700 font-semibold mb-2 text-sm md:text-base">Gambar Produk</label>
        ...
        <div class="mb-6 md:mb-8">
            <label class="flex items-center cursor-pointer">
                <input type="checkbox" name="is_active" value="1" checked class="w-4 h-4 md:w-5 md:h-5 text-red-800 rounded focus:ring-2 focus:ring-red-800">
